    ...
    public class SceneLoaderServiceFacade : ISceneLoaderService
    {
        ...
        public async UniTask LoadScenesAsync(List<string> scenesToLoad, string activeScene, bool unloadRedundant) 
        {
            ...
            if (scenesNotInAddressables.Any() is false) {
                await _addressablesSceneLoaderService.LoadScenesAsync(scenesToLoad, activeScene, unloadRedundant);
            ...
            await _addressablesSceneLoaderService.LoadScenesAsync(scenesToLoad.Except(scenesNotInAddressables).ToList(), activeScene, unloadRedundant);
            SetActiveScene(activeScene);
        }
        ...
        {
            List<string> scenesNotInAddressables = scenesToUnload.Except(Address.Scenes.AllKeys).ToList();
            ...
            await _addressablesSceneLoaderService.UnloadScenesAsync(scenesToUnload.Except(scenesNotInAddressables).ToList());
        }
    }
}