        ...
        public async UniTask LoadScenesAsync(List<string> scenesToLoad, string activeScene, bool unloadRedundant) 
        {
            ...
            foreach (string sceneName in scenesToLoad)
            {
                if (sceneName == activeScene)
                    await ReloadSceneAsync(sceneName); 
                else
                    await LoadSceneAsync(sceneName, false); 
            }
        }

        public async UniTask ReloadSceneAsync(string sceneToReload)
        {
            if (_loadedScenes.ContainsKey(sceneToReload)) 
                await UnloadSceneAsync(sceneToReload);

            await LoadSceneAsync(sceneToReload, false);
        }
